...
checkpoint archive will stay on the node it was created on.

This is another small step towards better integration of the checkpoint
and restore functionality into Kubernetes. In this KEP we still do not
want to talk about automatic container migration or even scheduler integration.
...
not a technical challenge but it requires further discussion how to integrate
it into Kubernetes.
...
Additionally checkpointing a container will be an operation, at least while
being an alpha feature, that is opt-in. To be able to checkpoint a container
it is necessary to either enable it per namespace or per pod.

The reason to be able to either enable it per namespace or per pod is to support
different use cases. Looking at the user story "Optimize Resource Utilization"
a infrastructure provider wants to be able to checkpoint any container without
specifying the opt-in label for each pod. There might be also a cluster where
the feature is not enabled per namespace but still a user or an admin wants to
enable checkpointing per pod. To be able to support different use cases
checkpointing needs to be opt-in on the namespace level as well as on the pod
level.

TODO: think about a feature gate name
...
Yes. By disabling the feature gate `TODO` again.
